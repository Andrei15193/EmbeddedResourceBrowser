...
Assembly.Directory.embedded file.txt
                ...
                CodeBlock("<EmbeddedResource Include=\"Directory.embedded file.txt\" LogicalName=\"Directory/embedded file.txt\"/>", new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase) { { "language", "xml" } }),
                Paragraph(
                    ...
                    InlineCode("."),
                    Text(" character which delimits the extension. For instance, the following resource "),
                    InlineCode("My.Assembly.Name.One.Two.Three.File.Extension"),
                    Text(" has the "),
                    InlineCode("One.Two.Three"),
                    Text(" name remained because the assembly name is "),
